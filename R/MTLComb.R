...
#
# Copyright (C) 2024  Dr. Han Cao (hank9cao@gmail.com)
# All rights reserved.
#
...
#' @param C2 A non-negative constant to improve the generalization performance
#' @param C A non-negative constant to promote the selection of same signatures
...
#' @author Dr. Han Cao
################################################################################
solve_L21 <- function (X, Y, lam, C, C2, ctasks,opts){
  ...
  LS_eval_gradient <- function(W, X, Y){
    grad_Ws=sapply(1:length(Y), function(k){(XtXr[[k]] %*% W[,k]  - XtYr[[k]])/nSubs[k]}) 
    funcVals=LS_eval_funVal(W, X, Y)
    return(list(grad_Ws=grad_Ws, funcVals=funcVals))
  }
  
  LS_eval_funVal <- function (W, X, Y){
    funcVal <- sum(sapply(1:length(Y), function(k){mean(0.5 * (Y[[k]] - X[[k]] %*% W[,k])^2)}))
    return(funcVal)
  }
  ...
  LR_eval_gradient <- function(W, X, Y){
    ...
    grad_Ws=2*sapply(rData, function(x)x$grad_w) 
    funcVals=2*sum(sapply(rData, function(x)x$funcVal)) 
    return(list(grad_Ws=grad_Ws, funcVals=funcVals))
  }
  ...
  eval_gradient = function(W){
    Wc=W[,ctasks, drop=F]; Wr=W[,rtasks, drop=F]
    rc = LR_eval_gradient(W=Wc, X=Xc, Y=Yc)
    rr = LS_eval_gradient(W=Wr, X=Xr, Y=Yr)
    Wnew=W;
    Wnew[,ctasks]=rc$grad_Ws
    Wnew[,rtasks]=rr$grad_Ws
    Wnew=Wnew + C * W %*% G %*% t(G) + C2 * W
    funcVals=rc$funcVals + rr$funcVals + C/2 * sum((W %*% G)^2) + C2/2 * sum(W^2)
  ...
  eval_funVal <- function (W){
    ...
    return(rc + rr + C/2 * sum((W %*% G)^2) + C2/2 * sum(W^2))
  }
...
#' @param C A non-negative constant to promote the selection of same signatures
#' @param C2 A non-negative constant to improve the generalization performance. The default value is 0 
...
#' @author Dr. Han Cao
################################################################################
MTLComb_Train = function(X=NULL, Y=NULL, nlambda=10, lam_ratio=0.01, lambda=NULL, C=0, C2=0, ctasks=NULL, intercept=F, 
  ...
  fit=list();fit$ws=list();fit$Obj=vector(); fit$C2=C2; fit$C=C; fit$ctasks=ctasks; fit$intercept=intercept
  ntasks=length(X)
  rtasks=setdiff(1:ntasks, ctasks)
  ...
  for(i in 1:length(lam_seq)){
    m=solve_L21(X=X, Y=Y, lam=lam_seq[i], C2=C2, C=C, opts=optsTrain, ctasks = ctasks)
...
################################################################################
#' @title k-fold CV for multiple tasks
#'
...
MTLComb_CV = function(X=NULL, Y=NULL, nfolds=10, lam_ratio=0.01, nlambda=10, lambda=NULL, ctasks=NULL, intercept=F, 
                      opts=list(init=0, maxIter=50, tol=0.001, ter=2), C2=0, C=0){
  library("pROC")
  
  #partition the subjects for CV
  ...
  for (i in 1:length(cvPar)){
    ...
    fit=MTLComb_Train(X=Xtrain, Y=Ytrain, nlambda=nlambda, lam_ratio=lam_ratio, ctasks=ctasks, opts=opts, C2=C2, C=C, lambda=lambda, 
                      intercept=intercept)
    ...
    auc_fold=rbind(auc_fold, sapply(yhatC, function(x)mean(sapply(1:length(ctasks), function(xx)as.numeric(pROC::auc(as.factor(Ytest[[ctasks[xx]]]), x[[xx]], quiet=TRUE))))))
...
################################################################################
#' @title k-fold CV for multiple outcomes
#'
#' @description Split the subjects of each task into k folds, training a MTLComb model on k-1 folds of data and test on the holding out fold. 
#' Repeat this procedure over all folds
#' 
...
#' @author Dr. Han Cao
################################################################################
CV_multi_outcomes = function(X, Y, nfolds=10, nlambda=200, lam_ratio=0.001, ctasks=4, opts=NULL, C=1, C2=0.1, intercept=F){
  library(fmsb)
  
  CVsplit = split( sample(c(1:nrow(X[[1]]))) , seq(1,nrow(X[[1]]),nrow(X[[1]])/nfolds))
  r2_Ave=0
  lam_seq = vector()
  ntasks=length(X)
  rtasks=setdiff(1:ntasks, ctasks)
  for(i in 1:length(CVsplit)){
    ...
    fitTrain=MTLComb_Train(X=Xtrain, Y=Ytrain, nlambda=nlambda, lam_ratio=lam_ratio, ctasks=ctasks, opts=opts, C=C, C2=C2, intercept=intercept)
    
    r2_fold=vector()
    for (z in 1:length(Xtest)){
      wz=sapply(fitTrain$ws, function(x)x[,z])
      score = Xtest[[z]] %*% wz
      if(is.element(z, ctasks)){
        r2_fold=rbind(r2_fold, apply(score, 2, function(x)NagelkerkeR2(glm(as.factor(Ytest[[z]])~x, family = binomial))$R2))
      } else{
        r2_fold=rbind(r2_fold, apply(score, 2, function(x)summary(lm(Ytest[[z]]~x))$r.squared))
      }
    }
    r2_Ave = r2_Ave + r2_fold
    lam_seq = rbind(lam_seq, fitTrain$lam_seq)
  }
  
  r2_Ave=r2_Ave/nfolds
  # par(mfrow=c(1,2))
  # plot(colMeans(r2_Ave[ctasks, , drop=F]), main="explained variance for classification tasks", xlab="lambda index", ylab="R2")
  # plot(colMeans(r2_Ave[rtasks, , drop=F]), main="explained variance for regression tasks", xlab="lambda index", ylab="R2")
  # par(mfrow=c(1,1))
  
  lambda.regress.min=colMeans(lam_seq)[order(colMeans(r2_Ave[ctasks, , drop=F]))[1]]
  lambda.classify.min=colMeans(lam_seq)[order(colMeans(r2_Ave[ctasks, , drop=F]), decreasing = T)[1]]
  lambda.weighted.min=length(rtasks)/ntasks*lambda.regress.min+length(ctasks)/ntasks*lambda.classify.min
  return(list(lambda.regress.min=lambda.regress.min, lambda.classify.min=lambda.classify.min, 
              lambda.weighted.min=lambda.weighted.min))
}
